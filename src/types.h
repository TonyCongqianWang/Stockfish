...
class Depth {
   ...
   public:
    ...
    template<int Scale = 1024>
    static constexpr Depth from_fixed(int val) {
        return Depth((val * DEPTH_GRANULARITY) / Scale);
    }
    ...
    // Delete implicit mixed int operations to enforce strict type safety
    Depth operator+(int) = delete;
    Depth operator-(int) = delete;
    Depth& operator+=(int) = delete;
    Depth& operator-=(int) = delete;
