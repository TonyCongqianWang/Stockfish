...
class AffineTransformArgmax {
   public:
    ...
    int propagate(const InputType* input) const {
        ...
        // Otherwise, bypass to horizontal SSE/NEON logic over the inputs to avoid VNNI layout corruption.
        if constexpr (OutputDimensions > 1 && (OutputDimensions % OutputSimdWidth == 0))
        {
