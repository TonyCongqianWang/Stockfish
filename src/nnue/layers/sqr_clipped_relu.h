...
class SqrClippedReLU {
   public:
    ...
    static constexpr std::uint32_t get_hash_value(std::uint32_t prevHash) {
        ...
        hashValue += prevHash;
        // omitted on purpose because not written by trainer (yet)
        // hashValue += WeightScaleBitsLocal;
        return hashValue;
    }
    ...
    void propagate(const InputType* input, OutputType* output) const {
        ...
        for (IndexType i = 0; i < NumChunks; ++i)
        {
            ...
            int16x8_t r0, r1;
            // Neon needs to shift by one more since it uses
            // Saturating Doubling Multiply High (doubling before shift by 16).
            r0 = vshrq_n_s16(vqdmulhq_s16(words0, words0), SimdShiftAmount + 1);
            r1 = vshrq_n_s16(vqdmulhq_s16(words1, words1), SimdShiftAmount + 1);
        ...
        (void)SimdShiftAmount;  // Silence unused variable warning when no SIMD implementation is enable.
