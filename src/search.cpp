...
bool Search::Worker::iterative_deepening() {
    ...
    while (rootDepth + ONE_PLY < Depth::from_int(MAX_PLY) && !threads.stop
           && !(limits.depth && mainThread && rootDepth >= Depth::from_int(limits.depth)))
    {
        rootDepth += ONE_PLY;
        ...
        int iterationReduction = threads.iterationReduction.load(std::memory_order_relaxed);
        if (iterationReduction > 0)
            searchAgainCounter++;
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            while (true)
            {
                // Adjust the effective depth searched with failedHigh and fractional iteration reduction
                Depth adjustedDepth =
                  std::max(ONE_PLY, rootDepth - Depth::from_int(failedHighCnt) - Depth::from_raw(iterationReduction));
        ...
        if (limits.use_time_management() && !threads.stop && !mainThread->stopOnPonderhit)
        {
            ...
            timeReduction = std::clamp(
              interpolate(double((rootDepth - lastBestMoveDepth).to_int()), 4.96, 18.79, 0.639, 1.712), 0.629,
              1.544);
            ...
            else if (!mainThread->ponder)
            {
                ...
                // 3-bucket reduction: 1.0 -> 0.25 -> 0.125 -> 0.0625 starting at 50%
                int reduction = 0;
                if (effectiveElapsedTime > totalTime * 0.90)
                    reduction = 15;  // 0.0625 ply step
                else if (effectiveElapsedTime > totalTime * 0.75)
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, const bool cutNode) {
    ...
    const bool     seekMate = rootDepth >= Depth::from_int(16) && std::abs(rootMoves[pvIdx].score) >= 2000;
    ...
    depth = std::min(depth, Depth::from_int(MAX_PLY - 1));
    ...
    assert(DEPTH_ZERO < depth && depth < Depth::from_int(MAX_PLY));
    ...
    if (priorReduction >= Depth::from_int(3) && !opponentWorsening)
        depth += ONE_PLY;
    if (priorReduction >= Depth::from_int(2) && depth >= Depth::from_int(2) && ss->staticEval + (ss - 1)->staticEval > 166)
        depth -= ONE_PLY;

    Depth depthFloor = Depth::from_int(depth.to_int());

    // Step 6. At non-PV nodes we check for an early TT cutoff
    if (!PvNode && !excludedMove && ttData.depth > depthFloor - (ttData.value <= beta) * ONE_PLY
        && is_valid(ttData.value)  // Can happen when !ttHit or when access race in probe()
        && (ttData.bound & (ttData.value >= beta ? BOUND_LOWER : BOUND_UPPER))
        && (cutNode == (ttData.value >= beta) || depth > Depth::from_int(4)))
    {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (!ttCapture)
                update_quiet_histories(pos, ss, *this, ttData.move, std::min((112 * depth).to_int(), 695));
        ...
        {
            if (depth >= Depth::from_int(7) && ttData.move && pos.pseudo_legal(ttData.move) && pos.legal(ttData.move)
    ...
    else if (!PvNode && !excludedMove && ttData.depth > depthFloor - (ttData.value <= beta) * ONE_PLY
             && is_valid(ttData.value) && ttData.bound != BOUND_EXACT
             && ttData.bound & (ttData.value >= beta ? BOUND_UPPER : BOUND_LOWER) && depth > Depth::from_int(5))
            ...
            if (err != TB::ProbeState::FAIL)
            {
                ...
                if (b == BOUND_EXACT || (b == BOUND_LOWER ? value >= beta : value <= alpha))
                {
                    ttWriter.write(posKey, value_to_tt(value, ss->ply), ss->ttPv, b,
                                   std::min(Depth::from_int(MAX_PLY - 1), depth + Depth::from_int(6)), Move::none(), VALUE_NONE,
                                   tt.generation());
    ...
    if (!PvNode && eval < alpha - (482 * depth * depth).to_int())
        return qsearch<NonPV>(pos, ss, alpha, beta);

    // Step 9. Futility pruning: child node
    // The depth condition is important for mate finding. It shouldn't be tuned.
    if (!ss->ttPv && depth < Depth::from_int(seekMate ? 6 : 19) && eval >= beta && (!ttData.move || ttCapture)
        && !is_loss(beta) && !is_win(eval))
    {
        Value futilityMult = std::min((Depth::from_int(45) + depth * 4).to_int(), 85);
        futilityMult -= 20 * !ss->ttHit;

        Value futilityMargin =
          (futilityMult * depth
           - Depth::from_int((2789 * improving + 335 * opponentWorsening) * futilityMult / 1024
    ...
    if (cutNode && ss->staticEval >= beta - (13 * depth + Depth::from_int(47 * improving - 365)).to_int() && !excludedMove
        && pos.non_pawn_material(us) && ss->ply >= nmpMinPly && beta >= -2000)
    {
        assert((ss - 1)->currentMove != Move::null());

        // Null move dynamic reduction based on depth
        Depth R = Depth::from_int(7 + std::max((ss->staticEval - beta) / 256, 0)) + depth / 3;
        ...
        {
            if (nmpMinPly || depth < Depth::from_int(16))
                return nullValue;
            ...
            nmpMinPly = ss->ply + 3 * (depth - R).to_int() / 4;
    ...
    if (!ss->followPV && !allNode && depth >= Depth::from_int(6) && !ttData.move)
        depth -= ONE_PLY;
        ...
        && !(is_valid(ttData.value) && ttData.value < probCutBeta))
    {
        ...
        Depth      probCutDepth = depth - Depth::from_int(improving ? 5 : 3);
        ...
        while ((move = mp.next_move()) != Move::none())
        {
            ...
            if (value >= probCutBeta)
            {
                ...
                ttWriter.write(posKey, value_to_tt(value, ss->ply), ss->ttPv, BOUND_LOWER,
                               probCutDepth + ONE_PLY, move, unadjustedStaticEval, tt.generation());
...
moves_loop:  // When in check, search starts here
    ...
    if ((ttData.bound & BOUND_LOWER) && ttData.depth >= depth - Depth::from_int(4) && ttData.value >= probCutBeta
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            if (moveCount >= (3 + (depth * depth).to_int()) / (2 - improving))
                mp.skip_quiet_moves();
            ...
            if (capture || givesCheck)
            {
                ...
                int margin = (177 * depth + Depth::from_int(captHist * 34 / 1024)).to_int();
            ...
            {
                int dIndex  = std::clamp(depth.to_int() - 1, 0, int(lmrDivisor.size()) - 1);
                int history = (*contHist[0])[movedPiece][move.to_sq()]
                            + (*contHist[1])[movedPiece][move.to_sq()]
                            + sharedHistory.pawn_entry(pos)[movedPiece][move.to_sq()];

                // Continuation history based pruning
                if (history < -(4136 * depth).to_int())
                    continue;
        ...
        if (!rootNode && move == ttData.move && !excludedMove && depth >= Depth::from_int(6 + ss->ttPv)
            && is_valid(ttData.value) && !is_decisive(ttData.value) && (ttData.bound & BOUND_LOWER)
            && ttData.depth >= depth - Depth::from_int(3) && !is_shuffling(move, ss, pos) && !seekMate)
        {
            Value singularBeta  = ttData.value - ((59 + 66 * (ss->ttPv && !PvNode)) * depth / 63).to_int();
            Depth singularDepth = Depth::from_int(newDepth.to_int() / 2);
            ...
            if (value < singularBeta)
            {
                ...
                int doubleMargin = -2 + 204 * PvNode - 152 * !ttCapture - corrValAdj
                                 - 1175 * ttMoveHistory / 114178 - (ss->ply > rootDepth.to_int()) * 38;
                int tripleMargin = 70 + 279 * PvNode - 188 * !ttCapture + 81 * ss->ttPv - corrValAdj
                                 - (ss->ply > rootDepth.to_int()) * 43;

                extension =
                  Depth::from_int(1 + (value < singularBeta - doubleMargin) + (value < singularBeta - tripleMargin));
            ...
            {
                ttMoveHistory << -(Depth::from_int(421) + 110 * depth).to_int();

                if (!ss->inCheck && value > ss->staticEval)
                {
                    const int bonus =
                      std::clamp(((value - ss->staticEval) * 177 * singularDepth / 1024).to_int(),
        ...
        if (allNode)
            r += r * 276 / (256 * depth + Depth::from_int(268)).to_int();

        // Apply the computed LMR
        if (depth >= Depth::from_int(2) && moveCount > 1)
        {
            ...
            Depth rDepth = Depth::from_fixed(r);
            Depth d = std::max(ONE_PLY, std::min(newDepth - rDepth, newDepth + Depth::from_int(2))) + (PvNode ? ONE_PLY : DEPTH_ZERO);
            ...
            if (value > alpha)
            {
                ...
                newDepth += (doDeeperSearch - doShallowerSearch) * ONE_PLY;
        ...
        else if (!PvNode || moveCount > 1)
        {
            ...
            value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 5234) * ONE_PLY - (r > 5487 && newDepth > Depth::from_int(2)) * ONE_PLY, !cutNode);
        }
        ...
        if (PvNode && (moveCount == 1 || value > alpha))
        {
            ...
            if (move == ttData.move
                && ((is_valid(ttData.value) && is_decisive(ttData.value) && ttData.depth > DEPTH_ZERO)
                    || ttData.depth > ONE_PLY))
        ...
        int inc = (value == bestValue && ss->ply + 2 >= rootDepth.to_int() && (int(nodes) & 14) == 0
        ...
        if (value + inc > bestValue)
        {
            ...
            if (value + inc > alpha)
            {
                ...
                if (value >= beta)
                {
                    ...
                    ss->cutoffCnt += (extension < Depth::from_int(2)) || PvNode;
                    assert(value >= beta);  // Fail high
                    break;
                }

                // Reduce other moves if we have found at least one score improvement
                if (depth > Depth::from_int(3) && depth < Depth::from_int(12) && !is_decisive(value))
                    depth -= Depth::from_int(3);
    ...
    if (bestValue >= beta && !is_decisive(bestValue) && !is_decisive(alpha))
        bestValue = (bestValue * depth.raw_value() + beta * DEPTH_GRANULARITY) / (depth.raw_value() + DEPTH_GRANULARITY);
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        bonusScale += std::min((59 * depth).to_int(), 420);
        ...
        const int scaledBonus = std::min((150 * depth - Depth::from_int(85)).to_int(), 1337) * bonusScale;
    ...
    if (!excludedMove && !(rootNode && pvIdx))
        ttWriter.write(posKey, value_to_tt(bestValue, ss->ply), ss->ttPv,
                       ...
                       : PvNode && bestMove ? BOUND_EXACT
                                             : BOUND_UPPER,
                       moveCount != 0 ? depth : std::min(Depth::from_int(MAX_PLY - 1), depth + Depth::from_int(6)), bestMove,
                       unadjustedStaticEval, tt.generation());
    ...
    {
        auto bonus =
          std::clamp(((bestValue - ss->staticEval) * (bestMove ? 12 : 18) * depth / 128).to_int(),
                     -CORRECTION_HISTORY_LIMIT / 4, CORRECTION_HISTORY_LIMIT / 4);
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    if (!PvNode && ttData.depth >= DEPTH_ZERO
        && is_valid(ttData.value)  // Can happen when !ttHit or when access race in probe()
        && (ttData.bound & (ttData.value >= beta ? BOUND_LOWER : BOUND_UPPER)))
        return ttData.value;
...
int Search::Worker::reduction(bool i, Depth d, int mn, int delta) const {
    int d0 = std::clamp(d.to_int(), 0, int(reductions.size()) - 1);
    int d1 = std::min(d0 + 1, int(reductions.size()) - 1);
    int scale0 = reductions[d0];
    int scale1 = reductions[d1];
    int dScale = scale0 + (scale1 - scale0) * d.phase_1024() / 1024;
    int reductionScale = dScale * reductions[mn];
...
void update_all_stats(const Position& pos,
    ...
    int bonus =
      std::min((133 * depth - Depth::from_int(81)).to_int(), 1487) + 364 * (bestMove == ttMove) + (ss - 1)->statScore / 28;
    int malus = std::min((968 * depth - Depth::from_int(235)).to_int(), 2244);
...
void SearchManager::output_pv(Search::Worker&           worker,
    ...
    for (usize i = 0; i < multiPV; ++i)
    {
        ...
        if (depth == ONE_PLY && usePreviousScore && i > 0)
            continue;

        Depth d = usePreviousScore ? std::max(ONE_PLY, depth - ONE_PLY) : depth;
